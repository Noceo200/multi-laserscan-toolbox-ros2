...
public:
    ...
    void fuseAndPublish() {
        ...
        if(raw_scans.size() > 0){
            ...
            for (const auto& pair1 : raw_scans) {
                std::string source_name = pair1.first;
                transformed_scans[source_name] = convert_raw_data(raw_scans[source_name],new_frame,std::stod(sources_config[source_name]["start_angle"]), std::stod(sources_config[source_name]["end_angle"]),std::stod(sources_config[source_name]["scan_angle_offset"]), stringToVector3(sources_var[source_name]["frame_trans_vector"]), stringToVector3(sources_var[source_name]["frame_rot_vector"]));
    ...
    void fuseScans(int resolution, sensor_msgs::msg::LaserScan &scan1,sensor_msgs::msg::LaserScan &scan2) {
        ...
        for (int i = 0; i < resolution; ++i) { 
            scan1.ranges[i] = std::min(scan1.ranges[i],scan2.ranges[i]); //we take the closest signal
        }
    }
